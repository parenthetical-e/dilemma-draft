...
In what sense can your mathmatical union be testied? Normally to test a model or theory of learning, one tests whetheer or not the learning algorithm appears to exist in the animal of interest. However our axioms, and their learning rule independence, means any rule will do. Testing, must therefore be qualitative, for which we have three strong predictions to make. 

% TODO - add Diss of Rosenburg and supporting data that exists. Add it in each number.

\begin{enumerate}
    \item \textbf{Invariant exploration strategy}. The presence of reward should not change the exploration strategy. This is tricky, because differing sense information is often associated with the prescence of absence of reward. This sense information would in many cases change learning and so change curius exploration. \emph{Auxiliary assumptions}. Learning is possible because there is some novelty ($\hat E > \eta$), and some observations available to learn from. These criteria rule out for example situations where Levy walks would be optimal \cite{Viswanathan2000}.
    \item \textbf{Information preference following reward abscence}. When reward is absent, an animal should immediatly choose information collection with it's next action. \emph{Auxiliary assumptions}. Learning is possible because there is some novelty ($\hat E > \eta$), and some observations available to learn from.
    \item \textbf{Deterministic search}. Read strictly, environmental noise is the only source of noise in our models exploration behavior. To evaluate this we musst move from population-level distribution analysis, to moment-by-moment predation of animal behavoir \cite{Peters2016,Mangalam2021}. \emph{Auxiliary assumptions}. perfect memory, no forgetting, or action/parameter/neural noise; In the casse of internal noise like the last examples, this noise should be independent of the search).
    \item  \textbf{Well defined pure-periods}. There should be well defined period of exploration and exploitation. \emph{Auxiliary assumptions}. The are no other behaviors, are driving factors, present. For example, learning fear, avoiding, or grooming.
\end{enumerate}

Of course the presence of any four criteria does not indicate our theory is at work, mechanisitically. Mechanisitic claims for any biological of behaavoiral problems are difficult if not impossible to be certian of. Instead, we view mmeeting each of these criterion as some evidence for our account. We think the nice mathematical properties of out curiosity trick make a good general candidate for solving explore-exploit problems. We wish to test how true that is. In the end, were ecological rationalists and not making perscptivist claims \cite{Rich2016a}.  
