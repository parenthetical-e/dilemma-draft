...
\begin{theorem}[State search: breadth] 
	\label{theorem:Z} 
...
\begin{theorem}[State search: depth] 
...
\begin{conjecture}[A hypothesis of equal important]
...
The WSLS rule in Eq~\ref{eq:pipi} when applied to exploration-exploitation problems can gauentee three things, shown below. In each Theoreum we assume our a finite horizon $T$ and that definitions for $\mathbf{X}$, $\mathbf{A}$, $\mathbf{M}$, $f$, $\Lambda$, and $\Pi_{\pi}$ are implicitly given.
