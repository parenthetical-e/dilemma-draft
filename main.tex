...
To the skeptical reader Axioms~\ref{ax:4} may seem too opinionated. A simpler natural set of Axioms is to keep 1-3 but replace~\ref{ax:4} with a new Axiom  that only requires information value track the total change in probability flux, that is $\text{E \sim dP$. For this metric, the curves in Figure~\ref{fig:metrics_specifity} overlap. We considered this, but rejected it after much contemplation. We feel this simpler view is incomplete. As theorists and scientists we favor more specific, therefore more actionable, information. 


% ---------------------------------------------------------------------------
\subsection*{Finding the optimal policy}
In studying systems for reinforcement learning it is standard to embed the learning problem into a Markov decision space. In this space, each state transition $s_t \rightarrow s_{t+1}$ is independent of the history of transition, and where each transition generates a reward $r$ again independent of the history of states. Our definition of information value $E$ violates  the formalism for $r$, in the $E$ does dependent on past states. However $E$ is calculated iteratively, given an $M$ and a pair pf states $(s, s'$, and it yields a non-negative value with each iteration (i.e $E_{(s,s')} \ge 0$.  This leaves open the possibility of finding an optimal iterative learning rule for maximizing using dynamic programming. 

We say at point in time $t$ an agent occupies a state $s$. In practice $s$ may be a as simple as their position, the current visual input, or any other combination of sense information. To keep the theory tractable, we assume that the total number of states an animal might visit $S$ is finite. Formally then, $s\ in \textbf{S}; \textbf{S} \in \mathbb{R}^N$, where we use real numbers to represent states and $N$ is the total number of states. Again to keep analysis the analysis tractable we limit the number of actions $a$ animal might take to finite real set, $a \in \textbf{A}; \textbf{A} \in \mathbb{R}^K$, where there are $K$ actions. 
...
A policy function and transition function combine to generate a path $P$.

\begin{definition}
    Let $P$ be a finite ordered collections of states $s$, such that $s \in S$ and the length of $|P| \leq \infty$. A path $P$ can also be defined recursively as, $P(t+1) \leftarrow \Lambda(P(t), \pi(P(t)))$.
\end{definition}

We use $t$ to index into $P$. 

Learned information is said to be stored in a finite perfect (i.e., noiseless) memory, denoted by $\textbf{M}$. 

\begin{definition}
    Let $M$ is probability distribution, we let $M$ be a table of independent states $S$ and actions $A$, such that $M \in (0, 1)^{N\text{x}K}$. % TODO: Do I need to formalize independent somehow?
\end{definition}

\begin{definition}
    Let $\ell$ be a memory function that maps a memory $M$ into itself, given some observed state $s$, $L : s, M \rightarrow M$. 
\end{definition}

The memory $M$ is update with a loss function $\mathcal{L}$, subject to $min \mathcal{L} (M, {s})$. 

\textit{With a loss of generality}, we assume that ${\mathcal{L}$ is convex, and that every observation $s$ leads to learning progress unless $L(s) = 0$. That is, the gradient of $L$ is, $\frac{dL_{\pi_{a}}}{ds} \leq 0$ for all $s \in P$. These are strong statements on the animal's ability to learn from it's world. They allow us to develop several critical theorems, but might seem overly restrictive.

We discuss our measure of information value $E$ in detail above. But for completeness: 

\begin{definition}
    Let $E$ be the information value between two successive memories, $M$ and $M'$ corresponding to states $s$ and $s'$, defined by the KL divergence so $E(s) := KL(L(s, M), M) = KL(M', M) = \sum_{s \in S} M'(s) \text{log} `\frac{M'(s)}{M(s)} $.     
\end{definition}
