...
We denote information value as $E$, subject to the following axiomatic constraints.
...
\end{axiom}
\begin{axiom}
    $E$ is strictly monotonic with the total change in $d$ measured between $M_{t+1}$ and $M_{t}$, for all $s \in S$.
    \label{ax:4}
\end{axiom}
\begin{axiom}
    $E$ is monotonic with the compactness $C$ of $d$ measured between $M_{t+1}$ and $M_{t}$, for all $s \in S$ (Eq.~\ref{eq:compactcude}).
    \label{ax:5}
\end{axiom}

 We ultimately want to address specificity of information in Axiom~\ref{ax:5}. To make the idea of specificity formal, we study the compactness of the memory; compactness has an established precise meaning. 
 
 The compactness $C$ of a hyper-cube has a simple formula, $C = \frac{P^2}{A}$ where $P$ is the cube's perimeter and $A$ is its area. Therefore as $M_{t}$ moves to $M_{t+1}$, the we can measure the distances $\{d_i, d_{i+1}, d_{i+2},\ldots d_{N}\}$ for all $s \in S^N$ and treat them as if they formed a $N$-dimensional hyper-cube. By measuring this imagined cube we arrive at a geometric estimate for change in compactness of $M$ (Eq.~\ref{eq:compactcude}).

\begin{equation} \label{eq:compactcude}
\begin{split}
    C & = \frac{\Big ( 2 \sum_{N}^{i} d_i \Big )^2}{\prod_{N}^{i} d_i}
\end{split}
\end{equation}

Formality complete we restate the axioms informally to illuminate their motivations.

\begin{statement}
    (Axiom 1.) Any subjective value depends \textit{only} on the subject's own experience. Information value about $s$ is subjective, so should depend only on how past experience is changed.
\end{statement}
\begin{statement}
    (Axiom 2.) Learning new information is always good, in principle. The consequences of the information may not always be good, but that's a separate issue.
\end{statement}
\begin{statement}
    (Axiom 3.) If you know something perfectly, there is no value in learning it again.
\end{statement}
\begin{statement}
    (Axiom 4.) If information value should track the total change in memory. 
\end{statement}
\begin{statement}
    (Axiom 5.) If the amount of total change is held constant, more specific information is more always more valuable than less specific information. $E_t$ should therefore scale itself with specificity, which we measure using the idea of geometrical compactness (Eq.~\ref{eq:compact}).
\end{statement}
