...
When placed in a new environment animals will explore it, even if no tangible rewards, like food and water, are expected \cite{Liu2019,Jaegle,Todd2015}. Sometimes they'ss explore when the it is certain that will lead to less reward \cite{ZheWang2019}. If a reward is expected though, an animal will generally explore and discover it \cite{Todd2015}. This gets interpreted as the animal exploring \textit{for} reward--to maximize reward \cite{Sutton2018}. Here we analyze an alternative: animals don't explore to maximize reward at all, they explore to maximize information--sometimes--about reward. 
...
Here we make a stronger conjecture than Schmidhuber's \cite{Schmidhuber1991}. We assert that information is valuable \textit{entirely} for its own sake. This lets us completely separate reward and information into two objectives, whose value we try to independently maximize. Estimating dual values this way is  less simple than having a single objective, but the increase in complexity comes with a substantial theoretical benefit. Exploration becomes a tractable mathematical problem, one we can solve deterministically.
