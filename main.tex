...
\begin{abstract}
    The exploration-exploitation dilemma is a fundamental but intractable problem in the learning and decision sciences. Here we challenge the common view of the dilemma--which focuses only on reward--by breaking it down into a tractable two-part problem, creating separate mathematical objectives for exploration and exploitation. To make exploration independent of reward we develop a new set of value axioms, inspired by Shannon's original approach to developing information theory. We then turn to field of computer science--borrowing methods from both dynamic programming and job scheduling--to develop optimal (greedy) algorithms which maximize both information value and reward. The cost of this solution is an increase in worst-case sample efficiency when compared to a equivalent reinforcement learning problem. The severity of the cost depends on the size of the environment and the rate of rewards.
\end{abstract}
...
Life is like a rat in a maze. When placed in an new maze a rat will generally explore it's new environment, visiting each arm. This exploration gets interpreted as curiosity, and is often formally modeled maximizing information gained.

% TODO math

If the maze is familiar and a reward--might--be found in one the arms, the rat will generally explore and discover it \cite{Todd2015}. This exploration gets interpreted as the animal exploring \textit{for} reward--to maximize reward \cite{Sutton2018}. 

% TODO math

The exploration-exploitation dilemma comes into play once the rat has found one rewarding arm. It could simply return to this arm every time it finds itself in this maze--exploiting it. Or the rat could explore the other arms, hoping to find a larger or more consistent result. Theoretical study of this simple problem has shown it is intractable. The unknowable nature--or partial observability--of what's behind the other arms make the problem ill-defined. Theoretical study has also shown though its possible to approximate an answer, by blending information and reward.

% TODO math

% TODO: beta is the problem. Dilemma is masked.
% The dilemma is 2 problems: opp cost, the unknown.

To try and make progress on the dilemma itself we made a challenging conjecture--phrased as question: ``What if the rat \textit{always} explores \textit{just} to maximize information?''. In some sense this is quite unintuitive. Why would an animal explore just out of curiosity if it wants a reward. However, if this conjecture were true the dilemma can be avoided. The problem in Eq TODO can be broken into two independent parts--maximizing reward (exploitation) and maximizing information (exploration). 

 %TODO math dual

Here we study this challenging conception, using it prove the exploration-exploitation dilemma does contain a tractable mathematical problem. One we can solve optimally, and to our surprise deterministically. To do this we borrow methods and ideas from Shannon's information theory, dynamic programming, and job scheduling.

% This should be the News & Views?
% Our contribution is threefold. Inspired by Shannon's orignal axiomatic formulation of information theory, we develop a set of 5 axioms to serve as principled basis for information value. Next we use methods from computer science--dynamic programming--to prove that optimal way to maximize information value is with the Bellman equation (TODO). Finally, we again borrow tools from computer science and show that balancing reward with information value is analogous to a classic job scheduling problem. Using this analogy we arrive at a simple greedy solution that optimally maximizes both reward and information value. 

% Here we offer an alternative view: animals don't explore to maximize reward at all, they explore to maximize information--sometimes--about reward. 

% Schmidhuber layed out the theoretical groundwork for our view of exploration. He conjectured that \textit{information about the world is valuable for its own sake} \cite{Schmidhuber1991} and argued that, \textit{in addition} to searching for rewards, animals seek to explore to build a model of the world. In doing this he said they are motivated by information itself, as a curiosity signal. In developing this theory, Schmidhuber made the seemingly parsimonious choice that when it comes time optimize, information and reward terms be combined into a single objective function (similar to our Eq.\ref{eq:total_r_I}). This mathematical form set the stage for decades of productive work on rewards, fictive rewards, and curiosity learning \cite{Pathak2017,Sutton1990,dayan1996exploration}. Still, to make progress on underlying problem of the dilemma we challenge the conception of the problem itself.
...
% ----------------------------------------------------------------------------
% \section*{Results}
% We've conjectured the exploration-exploitation dilemma contains two separate problems. Learning to maximising reward value, and learning to maximize information value. The value of rewards are well known. So the conjecture begs the question of how to value information, if we don't consider it a reward.
...
To state the axioms first we need to introduce notation for states $S$, actions $A$, and a memory $M$. These definitions are abstract, as we try and develop a theory of information value that is independent of any particular model of cognition or learning. To clarify these abstractions, we offer concrete examples for how they can be realized in practice. In doing so we also make clear just how (purposely) broad the proofs we develop are.

% --
% TODO: this needs to be 2-3 sentences in total
...
We maximize information value using the Belman equation. 
