\section*{Discussion}
We have offered a way around the exploration-exploitation dilemma. We proved the classic dilemma, found when optimizing exploration for reward value, can be resolved by exploring instead for curiosity. In this form the rational trade-off we are left with is solved with a classic strategy taken from game theory: the win-stay lose-switch strategy. When information is more valuable than rewards, be curious, otherwise seek rewards. We prove this rule has zero formal regret, when maximizing both reward and information value on equal terms.

Arguing the dilemma can be solved without regret is a controversial claim. So we will address some potential criticisms, as a series of questions. The final question explains how this approach can be tested.

\textbf{Why curiosity?} Our use of curiosity rests on three well established facts. 1. Curiosity is a primary drive in most, if not all, animal behavior \cite{Inglis2001}. 2. It is as strong, if not sometimes stronger, than the drive for reward \cite{Loewenstein1994,Kidd2015,Gottlieb2018}. 3. Curiosity as an algorithm is highly effective at solving difficult optimization problems \cite{Schmidhuber1991,Pathak2017,Stanton2018,Fister2019,Mouret2015,Colas2020,Cully2015,Pathak2017,Schwartenbeck2019,Laversanne-Finot2018}. 


\textbf{What about information theory?}
...
The cost of curiosities is indirectness, which can make it slower to converge. Our numerical simulation suggests this cost is not fatal for finding practical successes.


\textbf{Is this too simple?}
...
\textbf{Does value as a technical problem even make sense?}
...
\textbf{What about mutual information, or truth?}
...
\textbf{So you suppose there is always a positive value for learning of all fictions, disinformation, and deceptions?}
We must. This is our most unexpected prediction. Yet, logically, it is consistent with our analysis and we believe qualitatively consistent with the behavior of humans, and animals alike. The fact that humans consistently seek to learn false things calls we as scientissts accept that learning alone is a motivation for behavior. In other words, it is more than a heuristic \cite{Oudeyer2018a}.


\textbf{Was it necessary to build a general theory for information value just to describe curiosity?}
...
\textbf{Doesn't your definition of regret ignore lost rewards when the curiosity policy is in control?}
...
\textbf{But isn't curiosity impractical?}
It does seem curiosity is just as likely to lead away from a needed solution as towards it. Especially so if one watches children who are the prototypical curious explorers \cite{Sumner2019,Kidd2015}. This is why we limit curiosity with boredom, and counter it with a drive for reward collecting (i.e., exploitation). 

There is a useful analogy between science and engineering here. Science is an open-ended inquiry, and engineering often is a purpose driven enterprise. They each have their own pursuits, but they also learn from each other, often in alternating iterations \cite{Gupta2006}. It is their different objectives though which makes them such good collaborators.


\textbf{But what about curiosity on big problems?}
...
\textbf{Does this mean you are hypothesizing that boredom is actively tuned?}
Yes we are predicting exactly that.


\textbf{But can animals tune boredom?}
...
\textbf{How can an animal tune boredom for a new setting?}
...
\textbf{What about aversive values?}
...
If we also assume the fear/aversive term should take precedence over all other options when there is a tie, then biases should be set so $\phi > \rho > \eta$.

\textbf{What about other values?}
...
\textbf{Does regret matter?}
Another way around the dilemma is to ignore regret altogether. This is the domain of pure exploration methods, like the Gitten’s Index \cite{Gittins1979}, or probably-approximately correct approaches \cite{Valiant1984a}, or other bounded forms of reinforcement learning \cite{Brafman2002}. Such methods can guarantee that you find the best reward value actions, eventually. These methods do not consider regret, as we have. But we argue that regret is critical to consider when resources and time are scarce, as they are in most natural settings. 



\textbf{Is the algorithmic run time practical?}
...
\textbf{What predictions does this theory make?}
...
\begin{enumerate}
    ...
    \item  \textbf{Well defined pure-periods}. There should be a well defined period of exploration and exploitation. \emph{Auxiliary assumptions}. There are no other behaviors present. For example, learning fear, avoiding, or grooming.
\end{enumerate}
...
\subsection*{Summary}
We have argued one can put aside any intuitions that suggest a curious search is too inefficient to be practical for reward collection. We have shown it can be made very practical, and mathematically optimal. We have shown there is a zero-regret way to solve the dilemma, a problem that has seemed unsolvable. Simply, bee curious.