...
Put another way, if information should be agnostic to semantic considerations to ensure its generality as a concept \cite{Shannon1948a}, we reason information value should be agnostic for the same reason.
...
As stated at the begning of the Theory section, we want a Bellman solution for $\hat E$. As during reward collection the common way to arrive at Bellman solution is to assume a Markov decision space. This is a problem. For our definition of memory we assume it must have a natural long-term path dependence. This breaks the Markov assumption. To get around this we prove instead that exact forgetting, of the last observation, is another way to derive a Bellman solution.
...
It is common to focus on best case performance in simulation experiments, as we have above. However worst case performance is just important in judging the utility of an algorithm. We therefore examined the progression from our best case hyperparameters, to the worst case.  

Unlike idealized simulations, animals cannot know with perfect fidelity how their environment may change. They cannot perfectly pre-optimize their search parameters for every task, in other words. We have been using carefully chosen parameters. To test the robustness of our algorithm, and all other exploration algorithms we considered, we reexamined reward collection performance across all hyper parameters from our tuning set. Results for this are shown in Figure~\ref{fig:robust}. 
...
No matter the parameter choices, exploration-as-curiosity produced top-3 performance on both tasks in Figure.~\ref{fig:robust}a-bd. Most interesting to note is the performance on the worst 10 parameters. Here curiosity was markedly better, with substantially less variability. We can explain this last result in the following way. 
